...
$sql9="SELECT accounts.*, users.*, currency.cur FROM users 
    JOIN accounts ON accounts.id = users.account_id 
    JOIN currency ON currency.id = accounts.currency_id WHERE users.id ='$userid'";
$q9=$conn->query($sql9);
$red9=$q9->fetch_object();

$sql = "SELECT accounts.*, users.*, currency.cur FROM users 
    JOIN accounts ON accounts.id = users.account_id 
    JOIN currency ON currency.id = accounts.currency_id WHERE users.id !='$userid' AND accounts.currency_id = '$red9->currency_id'";
                    ...
                    <tbody>
                        <?php while ($line = $table->fetch_object()) {  ?>
                            <tr>
                                ...
                                <td><?php echo getCurValue($red9->cur); ?></td>
